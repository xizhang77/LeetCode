...
class Solution(object):
	...
	def dfs(self, image, sr, sc, m, n, visit):
		if [sr, sc] in visit:
			return 
		visit.append([sr, sc])
		if sr >= 1 and image[sr][sc] == image[sr - 1][sc]:
			self.dfs(image, sr - 1, sc, m, n, visit)
		if sr < m - 1 and image[sr][sc] == image[sr + 1][sc]:
			self.dfs(image, sr + 1, sc, m, n, visit)
		if sc >= 1 and image[sr][sc] == image[sr][sc - 1]:
			self.dfs(image, sr, sc - 1, m, n, visit)
	...
	def floodFill(self, image, sr, sc, newColor):
		...
		n = len(image[0])
		visit = []
		self.dfs(image, sr, sc, m, n, visit)
